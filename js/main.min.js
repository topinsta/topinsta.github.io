...
clearTimeout(loadMoreTimer),loadMoreTimer=setTimeout(function(){var top=($("body").hasClass("loading"),$(window).scrollTop()),h=$(document).height()-$(window).height();if(top>h-20&&!that.isLoading&&!that.loadingAll&&that.loadMedia(),that.videoPlaying){var videoTop=$(that.videoPlaying).offset().top,videoHeight=$(that.videoPlaying).height(),barHeight=$(".navbar").height(),p=videoTop+videoHeight;top+barHeight>p&&(that.videoPlaying.pause(),that.videoPlaying=null)}},300)}),$(".navbar .donate").on("click",function(e){e.preventDefault();var form=document.getElementById("form-donate");$(form).trigger("submit")})},Application.prototype.videoEvents=function(){for(var that=this,playEvent=function(element){element.addEventListener("play",function(){that.videoPlaying&&that.videoPlaying.pause&&that.videoPlaying.pause(),that.videoPlaying=this}),element.addEventListener("pause",function(){that.videoPlaying=null})},i=0;i<this.videos.length;i++){var id=this.videos.shift(),element=document.getElementById(id);playEvent(element)}};var Forms=function(formElement){this.form=$(formElement),this.captcha=grecaptcha.render("captcha-"+formElement.replace("form-",""),{sitekey:"6LeuegwTAAAAAKPsNQmK8lkrSVTe4ReZRYUcE8cD"}),this.base=new Base,this.bindEvents()};Forms.prototype.bindEvents=function(){var that=this;this.form.on("submit",function(e){e.preventDefault();var form=this,table=$(this).attr("name").replace(/form-/g,""),data={},error=[];$(this).serializeArray().map(function(response){data[response.name]=response.value,strValidate(response.name,response.value)||error.push(response.name)}),$(this).find(".error").removeClass("error"),console.log("forms");var captcha=grecaptcha.getResponse(that.captcha);if(error.length>0)for(var i=0;i<error.length;i++)$('[name="'+error[i]+'"]').addClass("error");else console.log(captcha),$.ajax({jsonp:"callback",url:"https://www.google.com/recaptcha/api/siteverify",data:{secret:"6LeuegwTAAAAAKPsNQmK8lkrSVTe4ReZRYUcE8cD",response:captcha},success:function(data){data.success&&that.base.add(table,data,function(){$(form).addClass("active"),$(form).next(".form-success").addClass("active")})}})}),this.form.parents(".modal").on("click","header a",function(){var form=$(this).parents(".modal").find("form")[0];that.formReset(form)})},Forms.prototype.formReset=function(element){element.reset(),$(element).find(".error").removeClass("error"),$(element).removeClass("active"),$(element).next(".form-success").removeClass("active")};var Base=function(){this.config={domain:"https://topsinta.firebaseio.com/web/data"},this.rootRef=new Firebase(this.config.domain)};Base.prototype.add=function(table,data,callback){if(table){var newData=this.rootRef.child(table);data&&this.validate(table,data)?newData.push(data,function(){callback&&callback()}):console.log("Error: data is invalid")}},Base.prototype.validate=function(table,data){var fieldsRules={contact:["name","email","message"],seeme:["name","email","instagram"]};if(data&&table){var fields=fieldsRules[table],checked=!0,num={fields:fields.length,data:Object.keys(data).length};return num.fields!==num.data?!1:($.each(data,function(key,value){-1===fields.indexOf(key)&&(checked=!1),strValidate(key,value)||(checked=!1)}),checked?!0:!1)}return!1};var strValidate=function(type,str){var rules={name:/^[a-zA-ZáéíóúâêîôûãõàèìòùÁÉÍÓÚÂÊÎÔÛÃÕÀÈÌÒÙçÇ ]+$/.test(str)&&str.length>0,email:/^(?:[\w-]+\.?\+?)*[\w-]+@(?:\w+\.)+\w+$/.test(str),message:/^[a-zA-ZáéíóúâêîôûãõàèìòùÁÉÍÓÚÂÊÎÔÛÃÕÀÈÌÒÙçÇ ]+$/.test(str)&&str.length>0,instagram:/^[a-zA-ZáéíóúâêîôûãõàèìòùÁÉÍÓÚÂÊÎÔÛÃÕÀÈÌÒÙçÇ ]+$/.test(str)&&str.length>0};return rules[type]},Modal=function(){this.modals=$(".modal"),this.overlayer=$("#overlayer"),this.trigger=$(".modal-trigger")};Modal.prototype.open=function(modalId){this.overlayer.addClass("active"),this.modals.removeClass("active"),$(modalId).addClass("active")},Modal.prototype.close=function(){this.modals.removeClass("active"),this.overlayer.removeClass("active")},window.Application=new Application}(window);